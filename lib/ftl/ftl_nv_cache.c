...
static void
compaction_process_pin_lba(struct ftl_nv_cache_compactor *compactor)
{
	...
	FTL_RQ_ENTRY_LOOP(rq, entry, rq->iter.count) {
		...
		if (entry->lba == FTL_LBA_INVALID) {
			ftl_l2p_pin_skip(dev, compaction_process_pin_lba_cb, compactor, pin_ctx);
		} else {
			ftl_l2p_pin(dev, entry->lba, 1, compaction_process_pin_lba_cb, compactor, pin_ctx);
		}
	}
}
...
compaction_process_read_entry(void *arg)
{
	...
	int rc = spdk_bdev_read_blocks(dev->nv_cache.bdev_desc, dev->nv_cache.cache_ioch,
...
compaction_process_read(struct ftl_nv_cache_compactor *compactor)
{
	...
	struct ftl_nv_cache *nv_cache = compactor->nv_cache;
	struct spdk_ftl_dev *dev = SPDK_CONTAINEROF(nv_cache, struct spdk_ftl_dev, nv_cache);
	struct ftl_rq_entry *entry, *next, *io;
	...
	FTL_RQ_ENTRY_LOOP(rq, entry, rq->iter.count) {
		...
		FTL_RQ_ENTRY_LOOP_FROM(rq, entry + 1, next, rq->iter.count) {
			if (next->addr == io->addr + io->bdev_io.num_blocks) {
...
ftl_nv_cache_process(struct spdk_ftl_dev *dev)
{
	...
	if (nv_cache->nvc_desc->ops.process) {
		nv_cache->nvc_desc->ops.process(dev);
	}
}
...
recover_open_chunk_close_chunk(struct spdk_ftl_dev *dev, struct ftl_mngt_process *mngt)
{
	...
	*chunk_md = *chunk->md;
	...
	ftl_md_persist_entry(md, get_chunk_idx(chunk), chunk_md, NULL,
...
recover_open_chunk_execute(struct spdk_ftl_dev *dev, struct ftl_mngt_process *mngt)
{
	...
	nvc->nvc_desc->ops.recover_open_chunk(dev, mngt, ctx->chunk);
}
